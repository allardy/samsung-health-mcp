/**
 * Export freshness check for the local Samsung Health export directory/CSVs.
 *
 * Returns the export mtime, days since export, an `is_stale` flag, and a
 ...
 * `samsung_health_export_freshness` workflow tool and inside
 * `samsung_health_connection_status` as a warning when the export is stale.
 */
import { inspectExportLocation, type ExportLocation } from "./samsung-health-export.js";
...
export interface ExportFreshness {
  ok: boolean;
  ready_for_samsung_health_export: boolean;
  export_path?: string;
...
export async function buildExportFreshness(
  ...
  if (!ready || location.mtime_ms === undefined) {
    return {
      ok: false,
      ready_for_samsung_health_export: false,
      ...
      recommendation:
        location.note ??
        "No Samsung Health export found. Run `samsung-health-mcp-server setup --export-path /path/to/samsung_health_export` or set SAMSUNG_HEALTH_EXPORT_PATH.",
  ...
  if (staleReason === "older_than_30d") {
    recommendation = `Consider re-exporting (>${STALE_DAYS_HARD} days old). On Samsung Galaxy: Samsung Health app → Settings → Download personal data → export, then transfer the new files.`;
  } else if (staleReason === "older_than_7d_and_no_recent_records") {
    recommendation = `Export is ${daysSinceExport} days old and the latest record in it is ${recentRecordDays} days old. Re-export from Samsung Health for fresh data.`;
  } else {
  ...
  return {
    ok: true,
    ready_for_samsung_health_export: ready,
    export_path: exportPath,
    export_kind: location.kind,
    exists: true,
    modified_at: location.modified_at,
...
export function formatExportFreshnessMarkdown(freshness: ExportFreshness): string {
  return [
    "# Samsung Health Export Freshness",
    "",
